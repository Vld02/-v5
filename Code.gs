...
const EDIT_CONFIG = Object.freeze({
  rules: Object.freeze({
    ...
    // Значение FILE хранит ссылку на загруженный файл. Сам файл передаётся
    // только через uploadDocumentAttachment(), а не через updateResultCell().
  ...
  fields: Object.freeze({
    ...
    // Для нового скана достаточно добавить заголовок таблицы в эту карту с rule: 'FILE'.
    // Такой заголовок будет отображён отдельной строкой с состоянием вложения.
...
/**
 * Загружает файл в общую папку вложений и сохраняет его URL в документном поле.
...
function uploadDocumentAttachment(formData) {
  ...
  if (!fieldConfig || (fieldConfig.rule !== 'FILE' && !fieldConfig.attachment)) {
  ...
  if (authorizedRowIndex === -1) throw new Error('Не удалось подтвердить пользователя для загрузки файла.');

  const folder = DriveApp.getFolderById(CONFIG.ATTACHMENTS_FOLDER_ID);
  const uploadedFile = folder.createFile(file);
  if (fieldConfig.rule === 'FILE') {
    const historyTimestamp = Utilities.formatDate(new Date(), CONFIG.TIMEZONE, `${CONFIG.DATE_FORMAT} HH:mm:ss`);
    const targetCell = sheet.getRange(authorizedRowIndex + 2, header.indexOf(columnName) + 1);
    setValueWithSiteEditNote_(targetCell, uploadedFile.getUrl(), historyTimestamp);
  }
  return { ok: true, fileName: uploadedFile.getName(), fileUrl: uploadedFile.getUrl(), attached: fieldConfig.rule === 'FILE' };
}
